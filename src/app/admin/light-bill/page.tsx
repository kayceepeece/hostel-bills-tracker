...
function LightBillContent() {
  ...
  const loadData = () => {
    ...
    fetch(`/api/light-bill?period=${encodeURIComponent(period)}`).then(r => r.json()).then(setPayments).catch(console.error);
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    await fetch(`/api/light-bill/${id}`, { method: 'DELETE' });
    loadData();
  };
...
export default function AdminLightBill() {
  return <AdminGuard><LightBillContent /></AdminGuard>;
}
